...
function install_node() {
    ...
    # 拉取指定的 Docker 镜像
    echo "正在拉取镜像 nillion/verifier:v1.0.0..."
    sudo docker pull nillion/verifier:v1.0.0
    ...
    # 运行 Docker 容器进行初始化
    sudo docker run -v ./nillion/verifier:/var/tmp nillion/verifier:v1.0.0 initialise
    ...
    if [ "$sync_status" = "yes" ]; then
        # 运行节点
        echo "正在运行节点..."
        docker run -d --name nillion_verifier -v ./nillion/verifier:/var/tmp nillion/verifier:v1.0.0 verify --rpc-endpoint "$selected_rpc_url"
        echo "节点正在运行。"
    else
        echo "节点未同步。脚本将退出。"
        exit 1
...
function change_rpc() {
    ...
    docker run -d --name nillion_verifier -v ./nillion/verifier:/var/tmp nillion/verifier:v1.0.0 verify --rpc-endpoint "$new_rpc_url" 
...
function view_credentials() {
    # 定义 JSON 文件路径
    credentials_file="$HOME/nillion/verifier/credentials.json"
...
function update_node() {
    docker stop nillion_verifier
    docker rm nillion_verifier
    docker run -d --name nillion_verifier -v ./nillion/verifier:/var/tmp nillion/verifier:v1.0.0 verify --rpc-endpoint "https://nillion-testnet-rpc.polkachu.com/" 

    mv $HOME/nillion/accuser/credentials.json $HOME/nillion/verifier/credentials.json
    docker restart nillion_verifier
